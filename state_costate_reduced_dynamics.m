function chidot = state_costate_reduced_dynamics(t,chi,m,u_func,params)
    
    % dynamics for 12-dimensional state+costate
    % state is R,V)
    % c is exhaust velocity (c = Isp*g0)
    
    % Check for length
    if length(chi) ~= 12
        error("State vector chi is wrong length, should be 12 dimensional.")
    end
    ...
    % Set what u is
    u = u_func(chi,m,params);
     %fprintf("u = %d\n",u);
     
    %control_accel = -lambdaVhat*u*amax/params.accel_norm;
    %control_accel = -lambdaVhat*u*Tmax/m/params.accel_norm;
    control_accel = -lambdaVhat*u*amax;
...
%     chidot(4:6) = g + h - (lambdaVhat*u*Tmax/m)/1000/L_EM*(T_EM/(2*pi))^2; % Vdot (nondimensionalize control acceleration)
    ...
    %fprintf("Magnitude of CR3BP accel is: %d\n",norm(g+h));
    
    chidot(7:9) = -G'*lambdaV; % lambdaR dynamics
    chidot(10:12) = -lambdaR - H'*lambdaV; % lambdaV dynamics
